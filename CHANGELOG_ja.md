...
- カット操作(`d`)を UNDO した時、復元が１バイト少なくなっている不具合を修正 (#84)

### 変更

- ボールドが目立ちすぎる端末があるので、次のような調整で列の表現を改善した (#82)
  - 16進数表示レイアウトを `hexdump -C` スタイル互換とした
  - 16進数表示でのボールドを廃止
- 選択表示を改善 (#83) :
  - デフォルトでは白文字青背景
  - `NO_COLOR` が設定されている時は色反転のみ
...
- 環境変数 `NO_COLOR` が定義されていたら、色を無効にするようにした (no-color.org 参照) (#73)
