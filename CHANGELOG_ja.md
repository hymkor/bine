...
- 選択モード（`v`）: `v` キーで選択範囲の開始/終了 (#56)
- ヤンク（`y`）: 選択領域を内部バッファへコピー。選択なしの場合はカーソル位置の1バイトをコピー (#56)
- 削除してヤンク（`d`）: 選択領域を削除して内部バッファへコピー。選択なしの場合は `x` と同じ動作 (#56)
- `p` / `P` が複数バイトのペーストに対応 (#56)
- 標準出力がリダイレクトされている場合、画面表示に標準エラー出力を使用するように変更 (#57)
- 保存先に `-` を指定すると標準出力へ書き出すように対応（標準出力が端末の場合はエラー） (#57)
- `a` でカーソル右へ１バイト挿入した後、その位置へカーソルを移動するようにした。(#62)
- 検索機能を追加 (#64)
  - `0xFE 0xFF` のようなバイト列や `"text"` のような文字列で検索可能
  - `/` および `?` により前方／後方検索
